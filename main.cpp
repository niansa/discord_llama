    ...
    CoSched::AwaitableTask<void> llm_init() {
        ...
        for (const auto& [model_name, model_config] : config.models) {
            ...
            if (model_config.is_non_instruct_mode_allowed() &&
                ...
                co_await llm->append(fmt::format(fmt::runtime(prompt), "bot_name"_a=bot.me.username, "bot_prompt"_a=model_config.bot_prompt, "user_prompt"_a=model_config.user_prompt), show_console_progress);
